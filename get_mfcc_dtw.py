...
def plotalignment(therapist_time_pitch, patient_time_pitch, path):
    ...
    if plot:
        ...
        for i, j in path:
            plt.plot((therapist_time_pitch[i, 0], patient_time_pitch[j, 0]),
                    (therapist_time_pitch[i, 1] + offset, patient_time_pitch[j, 1]), 'k')
        plt.tick_params(
...
def extract_pitch(wavfile, pitchfile, hop):
    ...
    """
    pitch_extract_cmd = 'Praat.app/Contents/MacOS/Praat --run ExtractPitch.praat ' + wavfile + ' ' + pitchfile + ' ' + str(
        pitch_ceiling) + ' ' + str(hop)
    os.system(pitch_extract_cmd)
    """
    input_file = wavfile
    output_file = pitchfile

    inputFN = 'example.wav'
    outputFN = 'example.PitchTier'
    praatEXE = '/Applications/Praat.app/Contents/MacOS/Praat'
    minPitch = 75
    maxPitch = 600
    # tg = tgio.openTextgrid(input_file)
    pitch = pitch_and_intensity.extractPitch(wavFN=input_file,outputFN=pitchfile, minPitch=minPitch, maxPitch=maxPitch, praatEXE=praatEXE)
    # extract Pitch using pitch_and_intensity.extractPitch() with parameters outputFN, inputFN, minPitch, maxPitch, frameStep, praatEXE
    # pitch = pitch_and_intensity.extractPitch(outputFN, wavfile, minPitch, maxPitch, 0.01, praatEXE)


    with open(output_file, "w") as f:
        for time, value in pitch:
            f.write("{:.4f}\t{:.4f}\n".format(time, value))
    # pdb.set_trace()
...
def adjust_files(ori_sig, test_sig, path, hop, hop_test, fs):
    ...
    for item in path:
        ...
        test_sig_mod.append(
            test_sig[item[1] * num_samp_per_frame_test:item[1] * num_samp_per_frame_test + num_samp_per_frame_test - 1])
...
def PitchDistanceComputation(original_time_pitch, test_time_pitch, title):
    ...
    if print_flag:
        print('\n' + title)
        print("Pitch Deviation (L2-norm of the deviations in rhythm-compensated pitch feature vector) = ",
              np.linalg.norm(
                  pitch_frame_disturbance, ord=2))
        print("Pitch Difference (Distance between rhythm-compensated pitch feature vectors) = ", distance)
        print("Pitch Deviation (L6-L2-norm of the deviations in rhythm-compensated pitch feature vector) = ",
              CalcPESQnorm(pitch_frame_disturbance))
...
def VibratoBoundariesCalc(time, viblikelihood1, thresh):
    print("array", np.array(viblikelihood1))
    print(thresh)
...
def VibratoFeatureCalc(vibrato_time_stamps, time_pitch, test_flag=0):
    ...
    for ind in range(np.shape(vibrato_time_stamps)[0]):
        ...
        rate = 1.0 / ((1.0 / (len(time_zerocrossings) - 1)) * sum(
            np.abs(time_zerocrossings[0:-1] - time_zerocrossings[1:])))
    ...
    # for multiple rows
        ...
        for row in vibrato_time_stamps:
            ...
            if vibrato_time_stamps_test.size == 0:
                ...
                vibrato_time_stamps_test = np.vstack((vibrato_time_stamps_test, np.array([elem1, elem2])))
    vib_features_test, vibrato_time_stamps_final_test = VibratoFeatureCalc(vibrato_time_stamps_test,
                                                                            test_time_pitch_oldest, test_flag=1)
...
def TestFileAdjust_forVibrato(original, test, NFFT):
    ...
    original_time_pitch1, test_time_pitch1 = adjust_pitch_tracks(original_time_pitch_oldest, test_time_pitch_oldest,
                                                                    path)

    vibrato_time_stamps, vibrato_features_original = VibratoDetection(original_time_pitch1)
    vibrato_time_stamps_test, vibrato_features_test = VibratoTest(test_time_pitch1, vibrato_time_stamps,
                                                                    vibrato_features_original)
...
def CalcPESQnorm(frame_disturbance):
    ...
    # L6-norm
    L6norm = []
    for ind in range(0, len(frame_disturbance), int(split_sec_interval_frames / 2)):
...
def ExtractVibratoFeatures(time_pitch):
    ...
    for i in range(len(pitch)):
        ...
        if len(time_zerocrossings) <= 1:
            ...
            rate.append(1.0 / ((1.0 / (len(time_zerocrossings) - 1)) * sum(
                np.abs(time_zerocrossings[0:-1] - time_zerocrossings[1:]))))
    ...
    return np.vstack(
        (np.array(viblikelihood)[np.newaxis].T, np.array(rate)[np.newaxis].T, np.array(extent)[np.newaxis].T))
...
def VolumeDistance(ori_sig, test_sig, rate):
    ...
    logenergy_ori = np.array(logenergy_ori).reshape(-1, 1)
    logenergy_test = np.array(logenergy_test).reshape(-1, 1)
...
def get_features(original:str, test:str):
    ...
    if print_flag:
        print("Rhythm Disturbance (L2-norm of the deviations in length-equalized mfcc feature vector) = ",
            np.linalg.norm(mfcc_frame_disturbance, ord=2))
        print("Timbral Difference (Distance between length-equalized mfcc feature vectors) = ", distance)
        print("Rhythm Disturbance (L6+L2-norm of the deviations in length-equalized mfcc feature vector) = ",
            CalcPESQnorm(mfcc_frame_disturbance))
    ...
    if print_flag:
        print(
            "Vibrato Disturbance (frame-based) (L2-norm of the deviations in length-equalized vibrato feature vector) = ",
            np.linalg.norm(vibrato_section_disturbance, ord=2))
        print("Vibrato Difference (frame-based) (Distance between length-equalized vibrato feature vectors) = ",
            distance)
        print(
            "Vibrato Disturbance (frame-based) (L2+L6-norm of the deviations in length-equalized vibrato feature vector) = ",
            CalcPESQnorm(vibrato_section_disturbance))
